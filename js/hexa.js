...
document.querySelector("#postForm").addEventListener("submit", (e) => {
  ...
  async function createPost() {
    ...
    let html = document.querySelector("#allPostsWrappear").innerHTML;

    let delete_posts_html = "";
    if (session_id === post.user_id) {
      delete_posts_html =
        '<button class="remove-btn" onclick="removeMyPost(this)">Remove</button>';
    }

    document.querySelector("#allPostsWrappear").innerHTML =
      `
    <div class="single-post" data-post_id="${post.id}">
      ...
      <div class="post-actions">
        ...
        <div>
          <button onclick="likePost(this)" class="likePostJS like-btn">
...
async function getAllPosts() {
  ...
  all_posts.forEach((post) => {
    async function getPostUser() {
      // Uzimamo usera koji je napravio post preko ida
      let user = new User();
      user = await user.get(post.user_id);

      // Pozivamo get sa IDem tog posta, gde u toj funk poredi sve ideve komentara, i tamo gde su isti, ubacuje ih u arr, i returnuje nam to vamo u coments
      let coments = new Commentt();
      coments = await coments.get(post.id);

      let comments_html = "";
      if (coments.length > 0) {
        coments.forEach((coments) => {
          comments_html += `<div class="single-comment">${coments.content}</div>`;
        });
      }
